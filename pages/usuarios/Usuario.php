...
class usuario {
  ...
  public function listarUsuario(){ //LISTA OS USUÁRIOS AO ABRIR PÁGINA
      $con = new Conexao();
      $pdo = $con->conectar();
  ...
  public function buscaAvancadaUsuario($busca){ //BUSCA AVANÇADA
  ...
  public function erroValidarUsuario($post){ //CASO HOUVE VALIDAÇÃO OS CAMPOS SÃO PREENCHIDOS
    ...
    if($post['ativo'] == 'S'){
      $s = 'selected="selected"';
    } else if($post['ativo'] == 'N') {
      $n = 'selected="selected"';
    }
    if(empty($post['id_usuario'])){ //NOVO CADASTRO ADICIONA O CAMPO DE SENHA
      $int_senhas = '<div class="col-md-5">
                      <div class="form-group">
                        <label>Senha*</label>
                        <input type="password" name="senha" class="form-control" id="ipt_senha" maxlength="32" required="required">
                      </div>
  ...
  public function validarUsuario($post){
    ...
    if (empty($post['id_usuario'])){ //VALIDA NOVO USUÁRIO
      ...
      if($stmt->execute()){ //VALIDA CAMPOS
        $msg = new Msg();
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
          if($row['email'] >= 1){ //VALIDA EMAIL CADASTRADO
            $msg->MsgErro('Email já cadastrado.');
            $this->erroValidarUsuario($post);
          } else if($post['senha'] !== $post['conf_senha']){ //VALIDA SENHAS
            $msg->MsgErro('Senhas não correspondem.');
            $this->erroValidarUsuario($post);
          } else { //CADASTRA NOVO USUÁRIO
            $this->cadastrarUsuario($post);
          }
        }
      } 
    } else {
      $this->cadastrarUsuario($post); //NOVO USUÁRIO
    }
  }
